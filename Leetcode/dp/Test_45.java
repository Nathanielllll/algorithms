...
也就是有 f[n - 1] = min(f[n - k],...,f[n - 3],f[n - 2]) + 1f[n−1]=min(f[n−k],...,f[n−3],f[n−2])+1。

然后我们再来考虑集合 f[n - k],...,f[n - 3],f[n - 2]f[n−k],...,f[n−3],f[n−2] 有何特性。
不然发现其实必然有 f[n - k] <= ...<= f[n - 3] <= f[n - 2]f[n−k]<=...<=f[n−3]<=f[n−2]。
